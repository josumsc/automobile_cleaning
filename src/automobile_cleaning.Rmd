...
Vamos a estudiar cómo afectan las marcas en los distintos parámetros, haciendo especial énfasis en el precio.
...
Por último, vamos a crear **n** grupos, uno por cada proveedor, para poder ver cómo la marca afecta a los distintos atributos (y principalmente al precio), por lo que realizaremos tres tipos de análisis:

- Correlación de los atributos con el precio según marca, comparando con la general.
- Predicción del precio con regresión lineal según marca y en general, comparando los resultados obtenidos entre marcas y con el general.
- Predicción del precio con regresión logística según marca y en general, comparando los resultados obtenidos entre marcas y con el general.
...
for (continent in continents) {
  dataset_by_continent[continent] = list(dataset[dataset$make.continent == continent, ])
}
dataset_by_continent
```
...
```{r message= FALSE, warning=FALSE}
# Recorremos cada una de las variables más interesantes de la dataset y cada uno de los continentes
continents <- c('asia', 'europe', 'america')
variables.to.test <- c('price', 'normalized.losses','horsepower','peak.rpm','engine.size')

for(variable in variables.to.test){
  a <- dataset_by_continent[['asia']][,variable]
  b <- dataset_by_continent[['europe']][,variable]
  print(bartlett.test(list(a,b)))
}

for(variable in variables.to.test){
  a <- dataset_by_continent[['asia']][,variable]
  b <- dataset_by_continent[['america']][,variable]
  print(bartlett.test(list(a,b)))
}

for(variable in variables.to.test){
  a <- dataset_by_continent[['europe']][,variable]
  b <- dataset_by_continent[['america']][,variable]
  print(bartlett.test(list(a,b)))
}
...
Para la primera prueba, la comparación de la correlación entre los atributos y el precio según la marca y de forma general, por lo que realizamos los cálculos y los guardamos para visualizarla y sacar conclusiones de ello en los siguientes apartados.

```{r message= FALSE, warning=FALSE}
# Calculamos las correlaciones
cuantitative_fields <- c('symboling', 'normalized.losses', 'wheel.base', 'length', 'width', 'height', 'curb.weight', 'engine.size', 'bore', 'stroke', 'compression.ratio', 'horsepower', 'peak.rpm', 'city.mpg', 'highway.mpg')
